    ...
    static _onConnect(port) {
        ...
        port.onMessage.addListener(async function(message) {
            let result = isPageLoadedMessage(message)
                ...
                : isPageView(message)
                ? (BackgroundApp._onPageView(port, message))
                : isTrackCustomEvent(message)
                ? (BackgroundApp._onTrackCustomEvent(port, message))
                : isCheckHealthMessage(message)
                ? (BackgroundApp._onCheckHealth(port, message))
                : isAppliedSuggestion(message)
                ? (BackgroundApp._onAppliedSuggestionMessage(port, message))
                : isLTAssistantStatusChangedMessage(message)
                ? (BackgroundApp._onLTAssistantStatusChangedMessage(port, message))
                : isCheckForPaidSubscriptionMessage(message)
                ? (BackgroundApp._onCheckForPaidSubscriptionMessage(port, message))
                : isTrackTextLengthMessage(message)
                ? (BackgroundApp._onTrackTextLengthMessage(port, message))
                : isTrackEventMessage(message)
                ? (BackgroundApp._onTrackEventMessage(port, message))
                : isOpenFeedbackFormMessage(message)
                ? (BackgroundApp._onOpenFeedbackFormMessage(port, message))
                : isSendFeedbackMessage(message)
                ? (BackgroundApp._onSendFeedbackMessage(port, message))
                : isOpenOptionsMessage(message)
                ? (BackgroundApp._onOpenOptionsMessage(port, message))
                : isOpenPrivacyConfirmationMessage(message)
                ? (BackgroundApp._onOpenPrivacyConfirmationMessage(port, message))
                : isCloseCurrentTabMessage(message)
                ? (BackgroundApp._onCloseCurrentTabMessage(port, message))
                : isValidateTextMessage(message)
                ? (BackgroundApp._onValidateTextMessage(port, message))
                : isLaunchEditorMessage(message)
                ? (BackgroundApp._onLaunchEditorMessage(port, message))
                : isGetValidatorDataMessage(message)
                ? (BackgroundApp._onGetValidatorDataMessage(port, message))
                : isStartDictionarySyncMessage(message)
                ? (BackgroundApp._onStartDictionarySyncMessage(port, message))
                : isAddWordToDictionaryMessage(message)
                ? (BackgroundApp._onAddWordToDictionaryMessage(port, message))
                : isBatchAddWordToDictionaryMessage(message)
                ? (BackgroundApp._onBatchAddWordToDictionaryMessage(port, message))
                : isRemoveWordFromDictionaryMessage(message)
                ? (BackgroundApp._onRemoveWordFromDictionaryMessage(port, message))
                : isClearDictionaryMessage(message)
                ? (BackgroundApp._onClearDictionaryMessage(port, message))
                : isGetPreferredLanguagesMessage(message)
                ? (BackgroundApp._onGetPreferredLanguagesMessage(port, message))
                : isLoadSynonymsMessage(message)
                ? (BackgroundApp._onLoadSynonymsMessage(port, message))
                : isUpdateDictionaryMessage(message)
                ? (BackgroundApp._onUpdateDictionaryMessage(port, message))
                : isOpenURLMessage(message)
                ? (BackgroundApp._onOpenURLMessage(port, message))
                : isOpenPremiumPageMessage(message)
                ? (BackgroundApp._openPremiumPage(port, message))
                : isLoginUserMessage(message)
                ? (BackgroundApp._loginUserMessage(port, message))
                : isLogoutUserMessage(message)
                ? (BackgroundApp._logoutUserMessage(port, message))
                : isOnLoginUserMessage(message)
                ? (BackgroundApp._onLoginUserMessage(port, message))
                : isOnLogoutUserMessage(message) && (BackgroundApp._onLogoutUserMessage(port, message));
            if (result instanceof Promise) {
                port.postMessage(await result);
            } else if (result) {
